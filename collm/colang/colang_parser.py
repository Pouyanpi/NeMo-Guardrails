...
class ColangParser:
    ...
    def _normalize_line_text(self):
        ...
        if self.mode == "colang":
            ...
            rules = [
                ...
                (r":$", r""),
                # Transform "..." into an auto_resume label or force_interrupt
                (r"^\.\.\.", f"label {ellipsis_label}_{self.current_line_idx}"),
                (r"^wait (.*)$", r'label wait "\1"'),
                (r"^wait\s*$", "label wait"),
                ...
                (r"^def ", r"define "),
                # transform "for each" in "for"
                (r"^for each ", r"for "),
                (r"^foreach ", r"for "),
                # tweak for topic definition
                (r"^define topic:", r"define topic "),
                ...
                # normalize calling flows i.e. add the `do` in front
                # for up to three return values
                (r"^(\$[\w.]+) = do ", r"do \1 = "),
                (r"^(\$[\w.]+)\s*,\s*(\$[\w.]+) = do ", r"do \1, \2 = "),
                (
                    r"^(\$[\w.]+)\s*,\s*(\$[\w.]+)\s*,\s*(\$[\w.]+) = do ",
                    r"do \1, \2, \3 = ",
                ),
                ...
                (
                    r"^done with (?:the )?(.*)$",
                    r'infer event done_with(intent_name="\1", intent_match=$latest_intent_match)',
                ),
                # Readable shorthands for list/dict manipulation
                (rf"^append (.+) to (.+)$", r"\2.append(\1)"),
                (rf"^add (.+) to (.+)$", r"\2.append(\1)"),
                (rf"^remove (.+) from (.+)$", r"\2.remove(\1)"),
                (rf"^extend (.+) with (.+)$", r"\1.extend(\2)"),
                (
                    rf"^update (.+) (?:in|for) (.+) (?:with|to) (.+)$",
                    r"\2.update({\1: \3})",
                ),
                (rf"^set (.+) (?:in|for) (.+) to (.+)$", r"\2.update({\1: \3})"),
                (
                    r"^set (\$[\w.]+) = (?:length|size|count) of (\$[\w.]+)",
                    r"set \1 = len(\2)",
                ),
                # TODO: this is slightly hackish for now
                # Turn $x.append(...) in $x = $x.append(...) or $x
                (rf"^({var_name}).append(.*)$", r"set \1 = \1.append\2 or \1"),
                (rf"^({var_name}).extend(.*)$", r"set \1 = \1.extend\2 or \1"),
                (rf"^({var_name}).remove(.*)$", r"set \1 = \1.remove\2 or \1"),
                (rf"^({var_name}).update(.*)$", r"set \1 = \1.update\2 or \1"),
            ]
    ...
    def _insert_topic_flow_definition(self):
        # Basically, we'll add few new lines
        # ```
        #   flow_topic_{topic_name}:
        #     user topic:{topic_name}
        # ```
        self.symbol_name, topic_object = extract_topic_object(self.symbol_name)
        p = self.current_line_idx + 1

        self.lines.insert(
            p,
            {
                "text": f"flow_topic_{self.symbol_name}_{topic_object}:",
        ...
        self.lines.insert(
            p,
            {
                "text": f"user topic:{self.symbol_name}",
                # We keep the line mapping the same
                "number": self.current_line["number"],
        ...
        if topic_object:
            if topic_object[0] == "$":
                # In this case we make sure the specified variable
                # contains the topic object which should also be set in $obj
                # (as the latest object in the context)
                self.lines.insert(
                    p,
                    {
                        "text": f"set {topic_object} = $obj",
                        # We keep the line mapping the same
                        "number": self.current_line["number"],
                        "indentation": self.next_line["indentation"],
                    },
                ...
                # In this case we just check that the specified object
                self.lines.insert(
                    p,
                    {
                        "text": f"context $obj == {topic_object}",
                        # We keep the line mapping the same
                        "number": self.current_line["number"],
                        "indentation": self.next_line["indentation"],
                    },
                ...
                raise Exception(f"Invalid topic object '{topic_object}'")

        # And we advance to the next line
        self.current_line_idx += 1
    ...
    def _apply_snippet(self):
        # If there's a snippet, we need to expand it at this point
        self.text = remove_token("use", self.text)

        # support both "use snippet X" and "use X"
        next_token = extract_main_token(self.text)
        if next_token == "snippet":
            self.text = remove_token("snippet", self.text)

        snippet_name = self._extract_snippet_name()
        for keyword in ["for", "with"]:
            if snippet_name.endswith(f" {keyword}"):
                snippet_name = snippet_name[0 : -1 * len(keyword)].strip()

        # Extract the params and get rid of the surrounding tags
        snippet_param_values = re.findall('(<[^>]*>|"[^"]*")', self.text)
        snippet_param_values = [
            param_value[1:-1] if param_value[0] in ['"', "<"] else param_value
            for param_value in snippet_param_values
        ]

        if self.snippets is None or snippet_name not in self.snippets:
            raise Exception(f"Unknown snippet '{snippet_name}'")

        _snippet = self.snippets[snippet_name]
        if len(_snippet["params"]) != len(snippet_param_values):
            raise Exception(
                f"Snippet '{snippet_name}' "
                f"was expecting {len(_snippet['params'])} parameters "
                f"and {len(snippet_param_values)} were given"
            )

        # We process all the lines, replace all parameters and append them.
        # Also, for the new lines we need to adjust the indentation so that
        # the first line of the snippet matches the current indentation
        indentation_diff = (
            self.current_indentation - _snippet["lines"][0]["indentation"]
        )

        p = self.current_line_idx + 1
        for snippet_line in _snippet["lines"]:
            snippet_line_text = snippet_line["text"]
            for j in range(len(snippet_param_values)):
                param_name = _snippet["params"][j]
                param_value = snippet_param_values[j]

                snippet_line_text = snippet_line_text.replace(
            ...
            self.lines.insert(
                p,
                {
                    "text": snippet_line_text,
                    # We keep the line mapping the same
                    "number": f"{self.current_line['number']}.snippet-{snippet_name}.{snippet_line['number']}",
                    "indentation": snippet_line["indentation"] + indentation_diff,
                },
            )
    ...
    def _extract_markdown(self):
        ...
        while self.current_line_idx < len(self.lines):
            ...
            if tokens[0] in [
                ...
                self.mode = "colang"
                return
            elif tokens[0] in ["use"]:
                self._apply_snippet()

                # Apply snippet will increment and we need to avoid double incrementation
                self.current_line_idx -= 1
            else:
    ...
    def _extract_params(self, param_lines: Optional[List] = None):
        ...
        yaml_block = "\n".join(yaml_lines)
        yaml_value = yaml.load(yaml_block)
    ...
    def _extract_params_old(self):
        ...
        while (
            ...
            name, value = get_stripped_tokens(split_max(self.lines[p]["text"], ":", 1))

            if value == "":
                # If it's an empty value, then it must be a list of items with
                # special indentation following the next line
                q = p + 1
                values = []
                while (
                    q < len(self.lines)
                    and self.lines[q]["indentation"] > self.lines[p]["indentation"]
                ):
                    values.append(self.lines[q]["text"].strip())
                    q += 1
                p = q - 1
                value = f"[{', '.join(values)}]"

            # if value is quoted, we remove the quotes
            if value[0] in ['"', "'"]:
                # there could also be more than one string
                # TODO: this only works for double quotes
                value = re.findall(r'"((?:\\"|[^"])+)"', value)
                if len(value) == 1 and name != "quick_replies":
                    value = value[0]
            ...
            # if it's a list, we use the python parser
            elif value[0] == "[":
                value = eval(value, {})
    ...
    def _parse_bot(self):
        ...
        if self.text.strip() == "bot":
            ...
            if text[0] == "{":
                self.current_element["bot"] = eval(text)
    ...
    def _parse_log(self):
        ...
        log_value = split_max(self.text, " ", 1)[1].strip()
        if log_value[0] in ["'", '"']:
            log_value = log_value[1:-1]
            ...
            "infer": [
                {
                    ...
                    "text": {
                        "elements": [
                            {
                                "text": log_value,
                                # TODO: this is a bit hacked, but we'll keep it for now
                                "style": "background: #ffffff; font-family: monospace; float: left; "
                                "margin-left: 50px; margin-top: -10px;",
                                "avatar": "https://static.roboself.com/bg/white.jpg",
                            }
                        ]
                    },
                }
    ...
    def _parse_flow(self):
        ...
        if self.text in ["resume", "resumes", "resuming"]:
            # This event will only be matched when a flow is resumed after
            # an interruption
            self.current_element = {
                "event": "flow_event",
                "this": not self.is_interruption_flow,
                "change": "resume",
            }

        elif self.text.startswith("interrupt") or self.text.startswith("digress"):
            self.current_element = {
                "event": "flow_event",
                "this": not self.is_interruption_flow,
                "change": "interrupt",
            }
            # We keep a reference to this as we need to match it with allow/deny
            self.last_event_flow_element = self.current_element

        elif self.text.startswith("abort"):
            self.current_element = {
                "event": "flow_event",
                "this": not self.is_interruption_flow,
                "change": "abort",
            ...
            raise Exception(f"Unknown flow event '{self.text}'")
    ...
    def _parse_allow(self):
        self.current_element = {
            ...
            "this": not self.is_interruption_flow,
            "change": "allow",
            "change_event": self.last_event_flow_element["change"],
        }

        # Add to current branch
        self.branches[-1]["elements"].append(self.current_element)

    def _parse_disallow(self):
        self.current_element = {
            "event": "flow_event",
            "this": not self.is_interruption_flow,
            "change": "deny",
    ...
    def _parse_var(self):
        """Just parsing global/local variable declarations for now.
        ...
        meta = branch_elements[0]
        if "variables" not in meta["meta"]:
            meta["meta"]["variables"] = {}
        ...
        for var_name in variables:
            ...
            default_value = None
            if "=" in var_name:
                var_name, default_value = char_split(var_name, "=")

            # Create the VariableInfo structure
            meta["meta"]["variables"][var_name] = {
                "name": var_name,
                "type": "implicit",
                "scope": scope,
    ...
    def _parse_param(self):
        """Just parsing parameters definition
        ...
        meta = branch_elements[0]
        if "parameters" not in meta["meta"]:
            meta["meta"]["parameters"] = {}
        if "variables" not in meta["meta"]:
            meta["meta"]["variables"] = {}

        parts = split_max(self.text, " ", 1)
        parameters = char_split(parts[1], ",")
        ...
        for param_name in parameters:
            ...
            default_value = None
            if "=" in param_name:
                param_name, default_value = char_split(param_name, "=")

            # Create the ParametersInfo structure
            meta["meta"]["parameters"][param_name] = {
                "name": param_name,
                "default_value": default_value,
            }

            # We also record a parameter as a local variable
            meta["meta"]["variables"][param_name] = {
                "name": param_name,
                "type": "implicit",
                "scope": "local",
                "default_value": default_value,
    ...
    def _parse_for(self):
        text = remove_token("for", self.text)
        parts = word_split(text, " in ")

        # Edge case: if there are more than two parts, we join the rest as there is probably
        # a list comprehension expression on the right
        if len(parts) > 2:
            parts = [parts[0], " in ".join(parts[1:])]

        var_name, values_expr = parts
        if var_name[0] == "$":
            var_name = var_name[1:]

        self.current_element = {"for": var_name, "in": values_expr, "do": []}
        self.branches[-1]["elements"].append(self.current_element)
    ...
    def _parse_stop(self):
        if not self.is_response_flow:
            self.current_element = {
                "stop": True,
            }
        else:
            self.current_element = {"bot": {"_type": "element", "text": "!<!"}}
        self.branches[-1]["elements"].append(self.current_element)
    ...
    def parse(self):
        while self.current_line_idx < len(self.lines):
            ...
            try:
                ...
                # If we're using a snippet, we apply it
                elif self.main_token == "use":
                    self._apply_snippet()
                    continue
                ...
                if self.main_token == "user":
                    ...
                    self._parse_bot()
                elif self.main_token == "log":
                    self._parse_log()
                elif self.main_token == "event":
                    self._parse_event()
                elif self.main_token in ["do"]:
                    self._parse_do()
                elif self.main_token in ["flow"]:
                    self._parse_flow()
                elif self.main_token in ["allow", "accept"]:
                    self._parse_allow()
                elif self.main_token in ["disallow", "deny", "reject"]:
                    self._parse_disallow()
                ...
                elif self.main_token in ["abort", "stop"]:
                    self._parse_stop()
                elif self.main_token in ["break"]:
                    self._parse_break()
                elif self.main_token in ["return", "done"]:
                    self._parse_return()
                elif self.main_token in ["var", "local", "global"]:
                    self._parse_var()
                elif self.main_token in ["param"]:
                    self._parse_param()
                elif self.main_token == "context":
                    # For context, we add a "_match" property to the current element
                    self.current_element["_match"] = split_max(self.text, " ", 1)[1]
                else:
                    raise Exception(
