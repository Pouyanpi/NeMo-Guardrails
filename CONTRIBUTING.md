...
NeMo Guardrails supports Python 3.10 through 3.13. Install Git and uv.
...
Valid optional extras are `sdd`, `eval`, `gcp`, `nvidia`, `tracing`,
`jailbreak`, `multilingual`, `server`, `chat-ui`, `hf-classifier`, and `all`.
For example:
