...
__all__ = [
    ...
    "KnowledgeBaseInitRails",
    "LLMActionCacheRails",
    "LLMActionRails",
    "RailsCheckRuntime",
    "StandardGenerationRails",
...
class HasEmbeddingSearchSeam(Protocol):
...
class HasEventsHistory(Protocol):
...
class LLMActionRails(HasConfig, HasRuntime, HasLLM, HasGenerationActions, Protocol):
    """Surface used by ``startup.llm_action_models`` (load + sync model bindings)."""


class LLMActionCacheRails(HasConfig, HasRuntime, Protocol):
    """Surface used by ``startup.llm_action_caches``."""


class GenerationActionRails(HasConfig, HasRuntime, HasLLM, HasGenerationActions, HasEmbeddingSearchSeam, Protocol):
    """Surface used by ``startup.generation_actions`` to register LLM actions."""


class KnowledgeBaseInitRails(HasConfig, HasRuntime, HasEmbeddingSearchSeam, HasKnowledgeBase, Protocol):
    """Surface used by ``startup.knowledge_base`` to build and register the KB."""


class ConversationEventRails(HasConfig, HasEventsHistory, Protocol):
    """Surface used by ``conversation.conversation_events``."""


class ColangTurnRails(HasConfig, HasRuntime, HasVerbose, Protocol):
    """Surface used by ``runtime.colang_turns`` to run a Colang turn."""


class StandardGenerationRails(ConversationEventRails, ColangTurnRails, HasExplainInfo, HasLogAdapters, Protocol):
    """Surface used by ``generation.generation_workflow`` (the standard generate pass)."""


class StreamingOutputRails(HasConfig, HasRuntime, HasLLM, HasExplainInfo, SupportsEnsureExplainInfo, Protocol):
    """Surface used by ``streaming.streaming_output_rails`` (output rails over a stream)."""


class GenerationStreamRails(StreamingOutputRails, SupportsGenerateAsync, Protocol):
