...
class GenerationRequestContext:
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            llm_stats_var.reset(self._llm_stats_token)
...
def start_generation_request_context(
    *,
    ...
    llm_stats_token = llm_stats_var.set(llm_stats_var.get())
    ...
    return GenerationRequestContext(
        ...
        _llm_stats_token=llm_stats_token,
        _streaming_handler_token=streaming_handler_token,
        _streaming_handler=streaming_handler,
    )


def set_generation_options_context(gen_options: Optional[GenerationOptions]) -> None:
    """Save generation options in the current async context."""
    generation_options_var.set(gen_options)


def bind_generation_request_context(
    *,
    messages: Optional[List[Dict[str, Any]]],
    streaming_handler: Optional[StreamingHandler],
) -> ExplainInfo:
    """Bind request-scoped generation context and return the active ExplainInfo."""
    if streaming_handler:
...
async def close_current_streaming_handler() -> None:
    """Close the streaming handler currently bound to the async context."""
    streaming_handler = streaming_handler_var.get()
    if streaming_handler:
        await streaming_handler.push_chunk(cast(Any, END_OF_STREAM))
